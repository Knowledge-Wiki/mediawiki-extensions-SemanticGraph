		...
		fetch(
			...
			.then((data) => {
				if (data.query.results[property].printouts["type"]) {
					...
					if (type === "http://semantic-mediawiki.org/swivt/1.0#_qty")
						deferred.resolve("quantity");
		...
		OO.ui.confirm("Submit changes?").done((confirmed) => {
			if (confirmed) {
				...
				for (const [key, page] of Object.entries(this.editNodes)) {
					...
					var promise = mwjson.api.updatePage(
						page,
						"Edited with SemanticGraph"
					);
					...
					promise.then(
						(page) => {
							...
							mwjson.api
								.purgePage(page.title)
								.then(() => mwjson.api.purgePage(page.title)); //Double purge
						},
						(error) => {
							error_occured = true;
							console.log(error);

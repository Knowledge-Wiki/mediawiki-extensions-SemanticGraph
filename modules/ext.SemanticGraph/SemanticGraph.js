...
isg.Graph = class {
					...
					if (selected_edge.from) {
						...
						if (selected_edge.label == "Category") {
							...
							li.addEventListener("click", () =>
								window.open(
									isg.util.articlePath( selected_edge.to )
								)
							);
							ul.prepend(li);
						} else {
	...
	saveNodeData(data, callback) {
		...
		if (
			...
			input_element.dataset.result !== "undefined"
		) {
			//existing page
			...
			data.url = isg.util.articlePath(data.id);
		}
		data.hidden = false;
		data.physics = false;
		input_element.value = "";
	...
	editEdgeWithoutDrag(data, callback) {
		...
		document.getElementById("edge-cancelButton").onclick =
			this.cancelEdgeEdit.bind(this, callback);
		$("canvas").on("click", (e) => {
	...
	async saveEdgeData(data, callback) {
		...
		if (
			!(
				this.contextCreatedProps.includes(data.label) ||
				this.config.properties.includes(data.label)
			)
		) {
		...
		if (isg.util.isLabelReversed(data.label)) {
			//reverseOrder
			sub = toNode;
			obj = fromNode;
		...
		if (property_type === "page") {
			...
			if (!this.data.editNodes[obj.id]) {
				//object page not handled yet
		...
		var page = undefined;
		if (!this.data.editNodes[sub.id]) {
			//subject page not handled yet
			...
			if (page.exists) {
				if (this.config.legacy_mode) {
					...
					if (
						...
						console.log(
							`WARNING: page ${sub.id} exits but is not a Semantic Term`
						);
		...
		if (this.config.legacy_mode) {
			mwjson.parser.append_to_template_param(
				page,
				"OslTemplate:KB/Term",
				["relations"],
				{
			...
			if (!page.slots["jsondata"]["statements"])
				page.slots["jsondata"]["statements"] = [];
	...
	async deleteSelectedEdge(data, callback) {
		...
		if (property_type === "quantity" || edgeToNode.isLiteral)
			obj = edgeToNode.label; //will never be reversed

		var page = undefined;
		if (!this.data.editNodes[sub]) {
			//subject page not handled yet
		...
		if (page.exists) {
			if (this.config.legacy_mode) {
				mwjson.parser.update_template_subparam_by_match(
					page,
					"OslTemplate:KB/Term",
					["relations"],
					{ property: property, value: obj },
					{}
				); //delete relation
				mwjson.parser.update_template_subparam_by_match(
					page,
					"OslTemplate:KB/Term",
					["relations"],
					{ property: "Property:" + property, value: obj },
					{}
				); //delete relation with property NS
			} else {
				if (mwjson.util.isString(page.slots["jsondata"]))
					page.slots["jsondata"] = JSON.parse(page.slots["jsondata"]);
				if (!page.slots["jsondata"]["statements"])
					page.slots["jsondata"]["statements"] = [];
				page.slots["jsondata"]["statements"] = page.slots["jsondata"][
					"statements"
